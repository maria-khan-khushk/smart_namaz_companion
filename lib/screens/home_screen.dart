...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    super.initState();
    // Load cached data immediately (synchronous-like but async)
    _loadCachedTimes();
    ...
    return Scaffold(
      ...
      body: Container(
        ...
        child: SafeArea(
          child: Column(
            children: [
              // Top row (no streak counter anymore – too heavy)
              Padding(
                padding: EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    ...
                    // Simple streak icon that navigates (no count shown here to save performance)
                    GestureDetector(
                      onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => StreakTrackerScreen())),
                      child: Container(
                        padding: EdgeInsets.symmetric(horizontal: 12, vertical: 6),
